...
// YUV formats.
#define wxAC_FOURCC_YUY2     wmcFOURCC('Y', 'U', 'Y', '2')  /* 16bit/pix YUV 4:2:2 as YUYV*/
...
void CAviLib::avi_writeindex(avi_handle *h)
{
    ...
    write(h->fd, &h->idx_hdr, sizeof(CHUNK_HDR));
    write(h->fd, h->idx_array, h->idx_index*sizeof(IDX_RECORD));
...
void CAviLib::avi_bigfile(avi_handle* h, int last)
{
    ...
    if (h->bigfile) {
		...
		// to file
		write(h->fd, &h->avix_hdr, sizeof(AVIX_HDR));
...
void CAviLib::avi_write_header(avi_handle* h)
{
    ...
    curpos = lseek(h->fd,0,SEEK_CUR);	// save location
    lseek(h->fd,0,SEEK_SET); // -->BOF

	write(h->fd, &h->avi_hdr, sizeof(AVI_HDR));// basic header
	if (h->video.fmtid != VIDEO_NONE) {
		write(h->fd,&h->avi_hdr_video,sizeof(AVI_HDR_VIDEO)); // v header
	}
	// there was audio...
	if (h->video.fmtid != VIDEO_NONE) {
		h->avi_hdr_odml.total_frames = h->frames_total;
		write(h->fd,&h->avi_hdr_odml,sizeof(AVI_HDR_ODML)); // odml header
    }

    write(h->fd, &h->avi_data, sizeof(AVI_DATA)); // data header
							  ...
							  int fp10s)
{
    ...
    if (-1 == (h->fd = open(h->file, O_BINARY | O_CREAT | O_RDWR | O_TRUNC, 0666))) {
    ...
    if (h->video.fmtid != VIDEO_NONE) {
		// having VIDEO
		streams++;
		v_rate=(pvideo.bytesperpixel * h->video.width*h->video.height * fp10s)/FPS_MULT; // video rate per second
	...
	// to file
    h->hdr_size += write(h->fd, &h->avi_hdr, sizeof(AVI_HDR)); //basic header
    ...
    if (h->video.fmtid != VIDEO_NONE) {
		...
		// to file
		h->hdr_size += write(h->fd, &h->avi_hdr_video, sizeof(AVI_HDR_VIDEO)); // v header
    }//if

	if (h->video.fmtid != VIDEO_NONE) {
		// to file
		h->hdr_size += write(h->fd, &h->avi_hdr_odml, sizeof(AVI_HDR_ODML)); // odml header
	}
	// header part completed

    /* data */
	// to file  - data header chunk
    if (-1 == write(h->fd, &h->avi_data, sizeof(AVI_DATA))) {	// data header
...
int CAviLib::avi_video(const video_buf &buf)
{
    ...
    if (-1 == write(h->fd, &h->frame_hdr, sizeof(CHUNK_HDR))) {
		fprintf(stderr,"write %s: %s\n",h->file,strerror(errno));
		return -1;
    }
    ...
    switch (h->video.fmtid) {
		...
		case VIDEO_BGR24_TD: {
			...
			for (y=0; y<(int)h->video.height; y++) {
				wx = write(h->fd, vcx->iov_base, (unsigned int)vcx->iov_len);
				if (-1 == wx) return -1;
				w += wx;
				vcx++;  // next scanrow
			}
